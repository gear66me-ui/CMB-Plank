...
(function(){
  ...
  findBtn.onclick = function(e){
    ...
    let v = '';
    try{
      if(window.cmb40r_capture_center) v = window.cmb40r_capture_center(false) || '';
    } catch(err) {}
    ...
    if(!v){
      setButton('NO<br>COORD', '#6a1b9a', '#fff');
      status('<b>No coordinate captured.</b> Wait for Aladin to finish loading or nudge the map slightly, then press FIND GAL again.', '#ce93d8');
      setTimeout(function(){
    ...
    if(!ok){
      status('<b>Colab callback bridge unavailable.</b> Coordinate is in the field above; use the lower Find Galaxy button.', '#ef9a9a');
    }
