    ...
    <div style="border:1px solid #78909c;border-radius:10px;overflow:hidden;background:#000">
      <div style="padding:8px 12px;background:#0b172a;color:white;font-family:sans-serif">
        ...
        <span style="font-size:12px;opacity:.85">
          Target: {html.escape(target_text)} · FOV {float(fov_deg):g}°
        </span>
      </div>
    ...
    if table is not None:
        for row in table:
            ...
            rows.append({
                ...
                "Angular size (arcsec)": safe_float(row["GALDIM_MAJAXIS"])
                if "GALDIM_MAJAXIS" in table.colnames else np.nan,
            })
    if not rows:
        return None, pd.DataFrame()
    frame = pd.DataFrame(rows).sort_values("Angular separation (arcsec)").reset_index(drop=True)
...
def details(obj):
    ...
    if np.isfinite(redshift) and redshift > 0:
        scale = (
            Planck18.kpc_proper_per_arcmin(redshift).to_value(u.kpc / u.arcmin)
            / 60.0 * 3261.563777
        )
        out.update({
...
def table_html(obj):
    keys = [
        "Object ID", "Catalog", "RA", "Dec", "Redshift", "Redshift type",
        "Light-travel time (Gyr)", "Comoving distance (Gly)",
        "Luminosity distance (Gly)", "Angular diameter distance (Gly)",
        "Universe age at emission (Gyr)", "Angular size (arcsec)",
        "Estimated physical diameter (ly)", "Stellar mass",
        "Star formation rate", "Morphology", "Type",
    ]
    rows = []
    for key in keys:
        ...
        if isinstance(value, (float, np.floating)):
            value = fmt(value)
        rows.append(
            f'<tr><th style="text-align:left;padding:5px 9px;background:#eef3f8">{html.escape(key)}</th>'
            f'<td style="padding:5px 9px">{html.escape(str(value))}</td></tr>'
        )
...
fov = widgets.Dropdown(
    options=[("0.01° — very tight", .01), ("0.03°", .03), ("0.08° — HUDF/GOODS", .08),
             ("0.25°", .25), ("1°", 1.0), ("5°", 5.0), ("20°", 20.0)],
    value=DEFAULT_FOV, description="Field:", layout=widgets.Layout(width="300px")
)
...
<b>Coverage note:</b> Hubble and JWST are pointed-observation archives, so blank fields may mean no coverage.
</div>
""")
...
def inspect_galaxy(_=None):
    ...
    try:
        ...
        <div style="padding:14px;border-radius:10px;background:#071a2b;color:white;font-family:sans-serif;line-height:1.8">
          ...
          You are observing this galaxy as it was:
          <b>{fmt(obj.get("Light-travel time (Gyr)"),3)} billion years ago</b><br>
          Current comoving distance:
          <b>{fmt(obj.get("Comoving distance (Gly)"),3)} billion light-years</b><br>
          Universe age when light was emitted:
          <b>{fmt(obj.get("Universe age at emission (Gyr)"),3)} billion years</b><br>
          Scale:
          <b>1 arcsecond = {fmt(obj.get("Scale (ly/arcsec)"),0)} light-years</b><br>
          Estimated galaxy diameter:
          <b>{fmt(obj.get("Estimated physical diameter (ly)"),0)} light-years</b>
        </div>
        """
        ...
        status.value = (
            f'<b style="color:#1b5e20">Inspection complete using {html.escape(obj["Catalog"])}: '
            f'{html.escape(str(obj["Object ID"]))}</b>{warning_html}'
        )
    except Exception as exc:
...
def save_galaxy(_=None):
    ...
    pd.DataFrame([row]).to_csv(
        CATALOG_PATH, mode="a", header=not CATALOG_PATH.exists(), index=False
    )
