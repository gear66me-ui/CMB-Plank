...
STATE = {'coord': None, 'object': None, 'nearby': pd.DataFrame(), 'warnings': [], 'last_aladin': None}
...
<div style="padding:12px 16px;border-radius:10px;background:#0b172a;color:white;font-family:sans-serif">
  ...
  <div style="font-size:13px;opacity:.9;margin-top:4px">Original Aladin-style map · drag to pan · pinch or use +/- to zoom · live center coordinate bridge</div>
</div>
''')
...
def _receive_aladin_coords(ra, dec):
    try:
        ra = float(ra)
        dec = float(dec)
        STATE['last_aladin'] = (ra, dec)
        coord_status.value = f'<b style="color:#1b5e20">Live Aladin center ready: {ra:.8f} {dec:.8f}</b>'
    except Exception as exc:
        coord_status.value = f'<b style="color:#b71c1c">Live coordinate bridge error: {html.escape(str(exc))}</b>'

try:
    from google.colab import output
    output.register_callback('cmb0039r.receive_aladin_coords', _receive_aladin_coords)
except Exception:
    pass

def embedded_aladin_html(target_text, fov_deg, survey_id):
    target_js = html.escape(str(target_text).strip(), quote=True)
    survey_js = html.escape(str(survey_id), quote=True)
...
<div style="width:100%;height:840px;background:#000;border:1px solid #333;position:relative;">
  <div id="cmb0039r_aladin" style="width:100%;height:840px;background:#000;"></div>
  <button id="cmb0039r_push" style="position:absolute;right:14px;bottom:14px;z-index:9999;background:#0ea5e9;color:white;border:0;border-radius:6px;padding:8px 12px;font:bold 13px sans-serif;">Send center to Galaxy Inspector</button>
  <div id="cmb0039r_ready" style="position:absolute;left:14px;bottom:14px;z-index:9999;background:rgba(0,0,0,.65);color:#e5e7eb;border-radius:6px;padding:7px 10px;font:12px sans-serif;">Loading Aladin…</div>
</div>
<script>
(function() {{
  ...
  function pushCenter() {{
    try {{
      if (!ready || !aladin) {{
        document.getElementById('cmb0039r_ready').innerText = 'Still loading; wait 2 seconds…';
        return;
      }}
      let xy = aladin.getRaDec();
      if (!xy || xy.length < 2) {{
        document.getElementById('cmb0039r_ready').innerText = 'Center not available yet; pan/zoom once.';
        return;
      }}
      google.colab.kernel.invokeFunction('cmb0039r.receive_aladin_coords', [xy[0], xy[1]], {{}});
      document.getElementById('cmb0039r_ready').innerText = 'Center sent: ' + Number(xy[0]).toFixed(8) + ' ' + Number(xy[1]).toFixed(8);
    }} catch(e) {{
      document.getElementById('cmb0039r_ready').innerText = 'Coordinate bridge waiting…';
    }}
  }}
  function initAladin() {{
    if (typeof A === 'undefined') {{
      document.getElementById('cmb0039r_ready').innerText = 'Waiting for Aladin library…';
      setTimeout(initAladin, 700);
      return;
    }}
    A.init.then(function() {{
      aladin = A.aladin('#cmb0039r_aladin', {{
        target: target,
        survey: survey,
        fov: fov,
        showReticle: true,
      ...
      ready = true;
      window.cmb0039r_aladin = aladin;
      document.getElementById('cmb0039r_ready').innerText = 'Aladin ready — pan/zoom, then press Send center.';
      setTimeout(pushCenter, 1200);
      try {{
        aladin.on('positionChanged', function() {{ setTimeout(pushCenter, 250); }});
      }} catch(e) {{}}
    }});
  }}
  document.getElementById('cmb0039r_push').onclick = pushCenter;
  setTimeout(initAladin, 300);
}})();
</script>
'''

def reload_map(_=None):
    viewer.value = embedded_aladin_html(target.value, fov.value, survey.value)
    coord_status.value = '<b style="color:#1565c0">Map loading. Wait a few seconds; the live center will appear here automatically.</b>'
...
def get_coordinates_from_aladin(_=None):
    if STATE.get('last_aladin') is not None:
        ra, dec = STATE['last_aladin']
        coord = SkyCoord(float(ra) * u.deg, float(dec) * u.deg, frame='icrs')
        _set_coord_fields(coord)
        coord_status.value = f'<b style="color:#1b5e20">Coordinates copied from live Aladin center: {ra:.8f} {dec:.8f}</b>'
        return
    try:
        coord = parse_target(target.value)
        _set_coord_fields(coord)
        coord_status.value = '<b style="color:#ef6c00">Live Aladin center has not arrived yet. Loaded current Target instead. Wait a few seconds or press Send center inside the map.</b>'
    except Exception as exc:
...
_set_coord_fields(parse_target(DEFAULT_TARGET))
reload_map()
