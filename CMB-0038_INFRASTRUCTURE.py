# CMB-0038_INFRASTRUCTURE.py
from pathlib import Path
import urllib.request
import runpy

SRC_URL = 'https://raw.githubusercontent.com/gear66me-ui/CMB-Plank/main/CMB-0034_INFRASTRUCTURE.py'
GENERATED = Path('CMB-0038_GENERATED_INFRASTRUCTURE.py')

print('CMB-0038 launcher-patcher')
...
text = text.replace('CMB-0034', 'CMB-0038')
text = text.replace('34A-1', '38A-1')
...
new = '''    clear_output(wait=True)
    ...
    <div style='border:1px solid #9e9e9e;border-radius:10px;padding:18px;background:#111827;color:#e5e7eb;font-family:Arial,sans-serif;'>
      <h3 style='margin:0 0 8px 0;color:#93c5fd;'>CMB-0038 Classic Viewer</h3>
...
# Hard correction: final diagnostics in source uses VIEWER["native"] is not None.
text = text.replace('VIEWER["native"] is not None', 'IPYALADIN_AVAILABLE and VIEWER["native"] is not None')

# Second correction: force native storage to remain None when ipyaladin is absent, even after refresh logic.
text = text.replace('VIEWER["native"] = aladin', 'VIEWER["native"] = aladin if IPYALADIN_AVAILABLE else None')

GENERATED.write_text(text, encoding='utf-8')
compile(text, str(GENERATED), 'exec')
print('Generated:', GENERATED)
print('Running generated CMB-0038 infrastructure...')
print('=' * 70)
