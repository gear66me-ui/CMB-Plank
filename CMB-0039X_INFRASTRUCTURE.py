...
def parse_icrs(value):
    value = str(value).strip()
    if not value:
        raise ValueError('Paste one ICRS coordinate string first, for example: 53.166625 -27.800000')
...
<div style="padding:12px 16px;border-radius:10px;background:#0b172a;color:white;font-family:sans-serif">
  ...
  <div style="font-size:13px;opacity:.9;margin-top:4px">Original Aladin touch map · left survey drawer preserved · Hubble Outreach color default</div>
</div>
''')
...
note = widgets.HTML(value='<div style="padding:9px 12px;border-left:4px solid #ffb300;background:#fff8e1;font:13px sans-serif"><b>Coordinate workflow:</b> Aladin is preserved as a remote iframe. Copy ICRS coordinates from Aladin and paste the complete RA Dec pair into the single field below.</div>')
...
coord_input = widgets.Text(value='', description='ICRS RA Dec:', placeholder='Paste one ICRS coordinate pair here, e.g. 53.166625 -27.800000', layout=widgets.Layout(width='760px'))
load_target_button = widgets.Button(description='Copy loaded target into ICRS cell', icon='copy')
inspect_button = widgets.Button(description='Find galaxy from ICRS coordinates', button_style='success', icon='search')
...
ra_dec_display = widgets.Text(description='Parsed:', disabled=True, layout=widgets.Layout(width='760px'))
...
def _set_coord_fields(coord):
    ...
    ra_dec_display.value = f'RA {coord.ra.deg:.8f} deg / Dec {coord.dec.deg:.8f} deg  |  sexagesimal {coord.ra.to_string(unit=u.hour, sep=":", precision=3)} {coord.dec.to_string(unit=u.deg, sep=":", precision=3, alwayssign=True)}'
...
def copy_loaded_target(_=None):
    try:
        coord = parse_icrs(target.value)
        _set_coord_fields(coord)
        coord_status.value = '<b style="color:#1565c0">Loaded target copied into the single ICRS RA Dec cell.</b>'
    except Exception as exc:
        coord_status.value = f'<b style="color:#b71c1c">Could not parse loaded target: {html.escape(str(exc))}</b>'

def inspect_galaxy(_=None):
    try:
        coord = parse_icrs(coord_input.value.strip() or target.value)
        _set_coord_fields(coord)
    except Exception as exc:
        STATE['coord'] = None
...
def save_galaxy(_=None):
    ...
    if obj is None or coord is None:
        ...
        return
    row = {'Date': pd.Timestamp.now().isoformat(), 'ICRS RA Dec': f'{coord.ra.deg:.8f} {coord.dec.deg:.8f}', 'Object ID': obj.get('Object ID', 'Unknown'), 'Redshift': obj.get('Redshift', np.nan), 'Distance Gly': obj.get('Comoving distance (Gly)', np.nan), 'Survey': selected_name(), 'Notes': notes.value}
...
def export_csv(_=None):
    if not CATALOG_PATH.exists():
        pd.DataFrame(columns=['Date', 'ICRS RA Dec', 'Object ID', 'Redshift', 'Distance Gly', 'Survey', 'Notes']).to_csv(CATALOG_PATH, index=False)
    display(FileLink(str(CATALOG_PATH)))

load_target_button.on_click(copy_loaded_target)
...
inspector = widgets.VBox([
    widgets.HTML('<h2 style="margin:12px 0 4px">GALAXY INSPECTOR — SINGLE ICRS COORDINATE CELL</h2>'),
    widgets.HTML('<div style="padding:8px 10px;background:#eef7ff;border-left:4px solid #1976d2;font:13px sans-serif"><b>Paste format:</b> one cell only, for example <code>53.166625 -27.800000</code> or <code>03:32:39.99 -27:48:00.0</code>.</div>'),
    widgets.HBox([coord_input]),
    widgets.HBox([load_target_button, inspect_button, save_button, export_button]),
    coord_status,
    ra_dec_display,
