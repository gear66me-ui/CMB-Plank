...
def cmb40r_capture_center(ra, dec):
    ...
    try:
        ...
        CENTER_CAPTURE["ra"] = ra
        CENTER_CAPTURE["dec"] = dec
        CENTER_CAPTURE["text"] = f"{ra:.8f} {dec:.8f}"

        print("[CENTER]", CENTER_CAPTURE["text"])
...
try:
    ...
    if output is not None:
        ...
        output.register_callback(
            "cmb40r.capture_center",
            cmb40r_capture_center
        )

        print("Registered callback: cmb40r.capture_center")

except Exception as exc:

    print("Callback registration failed:", exc)
...
def query_simbad(coord, radius_arcsec, rows, seen):
    try:
        Simbad.TIMEOUT = 45
        service = Simbad()
        service.add_votable_fields('otype', 'rvz_redshift')
    ...
    for row in table[:80]:
        try:
            ...
            add_row(rows, seen, coord, str(row['MAIN_ID']), 'SIMBAD', c.ra.deg, c.dec.deg, coord.separation(c).arcsec, row['RVZ_REDSHIFT'] if 'RVZ_REDSHIFT' in table.colnames else np.nan, str(row['OTYPE']) if 'OTYPE' in table.colnames else '')
        except Exception:
            pass
...
<div style="padding:12px 16px;border-radius:10px;background:#0b172a;color:white;font-family:sans-serif">
  ...
  <div style="font-size:13px;opacity:.9;margin-top:4px">Embedded Aladin Lite · Copy Center · live center readout · joystick FIND GAL sends crosshair center into Galaxy Inspector</div>
</div>
''')
...
viewer = widgets.Output(layout=widgets.Layout(width='100%'))


def reload_map(_=None):
    with viewer:
        clear_output(wait=True)
        display(HTML(viewer_html(target.value, fov.value, survey.value)))
...
find_button = widgets.Button(description='Find galaxy / object from ICRS coordinates', button_style='success', icon='search', disabled=False, layout=widgets.Layout(width='360px'))
...
status_line = widgets.HTML(value='')
progress = widgets.IntProgress(value=0, min=0, max=100, description='', bar_style='', layout=widgets.Layout(width='100%', visibility='hidden'))
...
def set_status(title, body, border='#2196f3'):
    ...
    if 'Searching' in str(title):
        find_button.disabled = True
    elif 'complete' in str(title).lower():
        find_button.description = 'Find galaxy / object from ICRS coordinates'
        find_button.disabled = False
        find_button.description = 'Find galaxy / object from ICRS coordinates'
    elif 'failed' in str(title).lower():
        find_button.description = 'Find galaxy / object from ICRS coordinates'
        find_button.disabled = False
...
def find_object(_=None):
    find_button.disabled = True
    find_button.icon = 'spinner'
    find_button.description = 'Searching…'
    find_button.button_style = 'warning'
    progress.layout.visibility = 'visible'
    progress.value = 2
    progress.bar_style = 'info'
    status_line.value = '<div style="color:#ffcc80;font-family:Arial,Helvetica,sans-serif;font-size:13px;padding:4px 0;">Starting search…</div>'
    results.value = ''
    nearby_panel.value = ''
    archives.value = ''
    STATE['warnings'] = []
    try:
        ...
        set_status('Searching inclusive catalog stack', parsed + '<br><b>Starting…</b> Querying NED, SIMBAD, MAST, and VizieR…', '#2196f3')
        radius_arcsec = max(600.0, min(7200.0, float(fov.value) * 3600.0))
        all_rows, seen = [], set()

        progress.value = 15
        status_line.value = '<div style="color:#90caf9;font-family:Arial,Helvetica,sans-serif;font-size:13px;padding:4px 0;">Querying NED…</div>'
        query_ned(coord, radius_arcsec, all_rows, seen)

        progress.value = 35
        status_line.value = '<div style="color:#90caf9;font-family:Arial,Helvetica,sans-serif;font-size:13px;padding:4px 0;">Querying SIMBAD…</div>'
        query_simbad(coord, radius_arcsec, all_rows, seen)

        progress.value = 55
        status_line.value = '<div style="color:#90caf9;font-family:Arial,Helvetica,sans-serif;font-size:13px;padding:4px 0;">Querying MAST…</div>'
        query_mast(coord, radius_arcsec, all_rows, seen)

        progress.value = 75
        status_line.value = '<div style="color:#ffcc80;font-family:Arial,Helvetica,sans-serif;font-size:13px;padding:4px 0;">Querying VizieR… this is usually the slowest stage.</div>'
        query_vizier_catalogs(coord, radius_arcsec, all_rows, seen)

        progress.value = 92
        status_line.value = '<div style="color:#90caf9;font-family:Arial,Helvetica,sans-serif;font-size:13px;padding:4px 0;">Sorting and selecting best match…</div>'
        ...
        all_rows = sorted(
            all_rows,
            key=lambda r: safe_float(r.get('Angular separation (arcsec)')) if np.isfinite(safe_float(r.get('Angular separation (arcsec)'))) else 999999
        )
        best = all_rows[0]
        STATE['object'] = best
        ...
        status_line.value = '<div style="color:#81c784;font-family:Arial,Helvetica,sans-serif;font-size:13px;padding:4px 0;">Search complete.</div>'
        find_button.button_style = 'success'
    except Exception as exc:
        set_status('Find galaxy failed', str(exc), '#f44336')
        progress.bar_style = 'danger'
        status_line.value = f'<div style="color:#ef9a9a;font-family:Arial,Helvetica,sans-serif;font-size:13px;padding:4px 0;">Search failed: {esc(str(exc))}</div>'
        find_button.button_style = 'danger'
    finally:
        find_button.disabled = False
        find_button.icon = 'search'
        find_button.description = 'Find galaxy / object from ICRS coordinates'
...
try:
    if output is not None:
        output.register_callback('cmb40r.findGalaxy', cmb40r_find_galaxy)
except Exception as exc:
    print('FindGalaxy callback registration failed:', exc)
...
(function(){
  ...
  document.getElementById('cmb40r_up').onclick = function(e){
    e.preventDefault(); e.stopPropagation(); scrollByPixels(-360);
  };

  document.getElementById('cmb40r_down').onclick = function(e){
    e.preventDefault(); e.stopPropagation(); scrollByPixels(360);
  };

  document.getElementById('cmb40r_pgdn').onclick = function(e){
    e.preventDefault(); e.stopPropagation();
    scrollByPixels(Math.max(520, Math.floor(window.innerHeight * 0.78)));
  };

  document.getElementById('cmb40r_hide').onclick = function(e){
    e.preventDefault(); e.stopPropagation(); dock.style.display='none';
  };

  function setButton(text, bg, color){
    findBtn.innerHTML = text;
    findBtn.style.background = bg;
    findBtn.style.color = color || '#fff';
  }
  ...
  function status(msg, color){
    ...
    if (ro){
      ro.innerHTML = msg;
      ro.style.color = color || '#e0e0e0';
    }
  }

  function invokeFind(v){
    try{
      if(window.google && google.colab && google.colab.kernel && google.colab.kernel.invokeFunction){
        google.colab.kernel.invokeFunction('cmb40r.findGalaxy', [v], {});
        return true;
      }
    } catch(e) {}
    return false;
  ...
  findBtn.onclick = function(e){
    ...
    setButton('USING<br>TARGET', '#1565c0', '#fff');
    status('<b>Iframe mode:</b> using the loaded Target field for Galaxy Inspector.', '#90caf9');

    let v = '';
    try{
      const inputs = Array.from(document.querySelectorAll('input[type="text"]'));
      const targetInput = inputs.find(el => (el.value || '').trim().length > 0);
      if(targetInput) v = (targetInput.value || '').trim();
    } catch(err) {}

    if(!v){
      setButton('NO<br>TARGET', '#6a1b9a', '#fff');
      status('<b>No target value found.</b> Use the lower Galaxy Inspector Find button.', '#ce93d8');
      setTimeout(function(){
        setButton('FIND<br>GAL', '#1b5e20', '#e8ffe8');
      }, 2200);
      return false;
    }

    setButton('FINDING<br>GAL', '#ff9800', '#111');
    status('<b>Using target:</b> ' + v + '<br>Sending to Galaxy Inspector…', '#81c784');
    ...
    if(!ok){
      status('<b>Callback bridge unavailable.</b> Use the lower Find Galaxy button.', '#ef9a9a');
    }
